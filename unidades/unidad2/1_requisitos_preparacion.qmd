...
```bash
# En Linux o macOS
sha256sum ubuntu-22.04.4-live-server-amd64.iso > descargado.sha256  # <1>
```
...
```powershell
(Get-FileHash ubuntu-22.04.4-live-server-amd64.iso -Algorithm SHA256).Hash  # <1>
```
...
```bash
sudo dd if=ubuntu-22.04.4-live-server-amd64.iso of=/dev/sdX bs=4M status=progress  # <3>
```
