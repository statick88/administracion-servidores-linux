...
$ multipass transfer archivo.txt abacom-dev:/home/ubuntu/  
```
...
2. **Siempre valida discos** - Usa **fdisk -l** para verificar disco correcto antes de "Erase"

3. **Deja espacio en / para crecer** - Mínimo 20 GB, preferible 50+ GB

4. **Separa /home de /** - Facilita actualizaciones sin perder datos

5. **Usa swap físico, no archivo** - Mejor performance que swapfile

6. **Documenta particiones** - Escribe en papel qué va en qué disco

7. **Practica en VirtualBox** - Antes de instalar en máquina real
...
8. Arrancar desde USB

9. Navegar el instalador de Ubuntu
...
Después de instalar Ubuntu, queremos conectarnos al servidor sin ingresar contraseña cada vez. Esto se logra con **llaves SSH**.

### Paso 1: Generar la Llave SSH

Desde tu computadora local (Mac, Windows o Linux):

```bash
$ ssh-keygen -t ed25519 -c "tu_email@ejemplo.com"
```

Cuando te pregunte la ruta, presiona **Enter** para usar la predeterminada:
...
1. **ed25519** es el algoritmo más seguro y rápido actualmente
2. Si no pones passphrase, la conexión será completamente automática
3. Se crean dos archivos: `id_ed25519` (privada) y `id_ed25519.pub` (pública)

### Paso 2: Copiar la Llave al Servidor
...
1. `ssh-copy-id` copia automáticamente tu llave pública al servidor
...
En la próxima lección, realizaremos la **configuración inicial del sistema** (usuarios adicionales, firewall).
