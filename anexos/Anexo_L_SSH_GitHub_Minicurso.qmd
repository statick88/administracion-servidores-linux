...
Si ya tienes experiencia con SSH y solo necesitas configurar la conexión con GitHub, sigue estos pasos:
...
- **`ls ~/.ssh/id_*`** lista las claves SSH existentes en tu sistema

**2. Generar clave Ed25519**

```bash
$ ssh-keygen -t ed25519 -C "tu@email.com"   # <1>
```

- **`ssh-keygen -t ed25519`** genera una clave con el algoritmo recomendado
- **`-C "tu@email.com"`** añade un comentario para identificar la clave

**3. Iniciar agente y agregar clave**
...
- **`ssh-add ~/.ssh/id_ed25519`** carga tu clave privada en el agente
...
- **`cat ~/.ssh/id_ed25519.pub`** muestra la clave pública (copia toda la salida)

**5. Agregar en GitHub**

Ve a **GitHub → Settings → SSH and GPG keys → New SSH key** y pega la clave pública.
...
- **`ssh -T git@github.com`** prueba la autenticación con GitHub

Si ves `Hi usuario! You've successfully authenticated...`, ¡todo funciona! 🎉
:::
...
- **pbcopy** copia la clave pública al portapapeles de macOS
...
- **xclip -selection clipboard** copia la clave pública al portapapeles de Linux
...
- **gh auth login** inicia sesión en GitHub desde la terminal con autenticación interactiva
...
- **gh ssh-key add --type authentication** agrega la clave para autenticación SSH

**Agregar con título:**

```bash
$ gh ssh-key add ~/.ssh/id_ed25519.pub --title "MacBook Pro 2024"   # <2>
```

- **--title** asigna un nombre descriptivo para identificar la clave en GitHub

**Para firma de commits:**

```bash
$ gh ssh-key add ~/.ssh/id_ed25519.pub --type signing   # <3>
```

- **--type signing** agrega la clave para firma de commits (funcionalidad separada)
...
- **ssh-keygen -t ed25519-sk** genera una clave respaldada por hardware FIDO2
...
- **ssh-keygen -t ecdsa-sk** es una alternativa compatible con claves más antiguas
...
- **ssh-add** agrega la clave de hardware al agente SSH para usarla en sesiones futuras
...
### 9.2 Opciones de configuración explicadas
...
### 11.6 Diagnóstico con `ssh -vvv` (modo verbose)
...
- **ssh -T** verifica la conexión con GitHub usando la clave de hardware
