...
sudo chown -R usuario:usuario /var/www/html/site  
exit

# Desde el servidor origen:
rsync /tmp/example.tar.gz usuario@192.168.56.10:/var/www/html/site/  
```
...
```bash
rsync /tmp/example.tar.gz usuario@192.168.56.10:~/site/  
```
...
```bash
rsync --rsync-path="sudo rsync" /tmp/example.tar.gz usuario@192.168.56.10:/var/www/html/site/  
```
...
sudo cp /home/usuario/mirepositorio/* /var/www/html/  
sudo nginx -s reload  
```
...
rsync -avz usuario@192.168.56.10:/usr/share/nginx/html/ ~/nginx-backup/  
```
...
sudo chown usuario:usuario /opt/backup  

# Transferir:
rsync -avz usuario@192.168.56.10:/usr/share/nginx/html/ /opt/backup/  
```
...
```bash
rsync -avz --preserve=mode,ownership origen/ destino/  
```
...
```bash
rsync -avz -e "ssh -i ~/.ssh/id_ed25519" origen/ usuario@192.168.56.10:~/destino/  
```
...
```bash
ssh-keygen -t ed25519 -C "usuario@laboratorio"  
```
...
**Servidor 1 (Origen):**
```bash
statick@ubuntu-server:~$ hostname -I | awk '{print $1}'  
192.168.56.10
```
...
```bash
statick@ubuntu-server:~$ cat /usr/share/nginx/html/index.html  
...
```bash
statick@ubuntu-server:~$ rsync -avz /usr/share/nginx/html/* statick@192.168.56.30:/var/www/html/site/  
ssh: connect to host 192.168.56.30 port 22: No route to host  
...
```bash
statick@ubuntu-server:~$ rsync -avz /usr/share/nginx/html/* statick@192.168.56.20:/var/www/html/site/  
statick@192.168.56.20's password:  
...
```bash
statick@ubuntu-server:~$ ssh statick@192.168.56.20  
sudo mkdir -p /var/www/html/site  
sudo chown -R statick:statick /var/www/html/site  
exit  
 <2> <3> <4>
```
...
```bash
statick@ubuntu-server:~$ rsync -avz /usr/share/nginx/html/* statick@192.168.56.20:/opt/backup/  
statick@192.168.56.20's password:  
sending incremental file list
index.html -> ../../../../home/statick/myrepository/index.html  
...
```bash
statick@ubuntu-server:~$ rsync -avz /usr/share/nginx/html/* statick@192.168.56.20:/opt/backup/backup.tar.gz  
statick@192.168.56.20's password:  
sending incremental file list
index.html -> ../../../../home/statick/myrepository/index.html  
...
- [ ] `/var/www/`
- [ ] `/usr/share/nginx/`
- [x] `/usr/share/nginx/html/`
- [ ] `/etc/nginx/html/`

### Pregunta 5: ¿Qué significa el error "No route to host"?

- [x] El servidor de destino no es alcanzable en la red
- [ ] El puerto SSH está cerrado
- [ ] La clave SSH no es válida
- [ ] Los permisos del archivo son incorrectos
:::

**Respuestas:**
4. **-a** archive (preserva permisos), **-v** verbose (muestra detalles), **-z** compress (comprime durante transferencia)
5. El usuario no tiene permisos de escritura en el directorio destino
6. **ping** verifica conectividad de red con el servidor destino
7. `/usr/share/nginx/html/` es la raíz de documentos de nginx en Ubuntu/Debian
8. El servidor de destino no es alcanzable (problema de red o IP incorrecta)